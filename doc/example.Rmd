...
This document shows how functions within run_method work in the gapctd package. batch processing works in the gapctd package.
...
## 7. Split data into upcast and downcast profiles

The make_cast_sections function uses the SBE Data Processing 'section' module to extract upcast and downcast data from CTD files. This is used instead of the SBE 'split' module to ensure profiles don't contain data collected while the trawl was being towed (which often results in bottom spikes in profiles due to changes in temperature and salinity along the bottom).

```{r split_casts, include=TRUE, eval=FALSE}
...
Batch processing routines do not handle all cases of salinity spikes. In this step, remaining 'bad' data are manually flagged by the analyst, removed, and bins that are removed are estimated using interpolation. Users can either specify which files to review using the 'file_paths' argument or retrieve all downcasts and upcasts and downcasts from a directory. Interpolation is performed using the method specified by the user (e.g., interpolation_method = "unesco"; see function documentation for details).
...
## 10. Review flagged and interpolated profiles

After manually reconciling sampling artifacts in the data, profiles are visually inspected on a haul-by-haul basis using the manual_flag_review() function to assess whether there are any remaining problems with the data. Hauls with problems remaining are removed and can be reprocessed manually using manual_flag_interpolate(), i.e., step #8.

```{r manual_review, include=TRUE, eval=FALSE}
gapctd::manual_flag_review(var = c("salinity", "temperature"), 
                           flag_dir = here::here("output", "manual_flag"))

```


## 11. Calculate surface temperature and salinity from each upcast
...
gapctd::calc_bin_value(accepted_dir = here::here("output", "accepted_profiles"),
                       z_var = "pressure",
                       z_range = 1,
                       agg_fun = mean)
```
