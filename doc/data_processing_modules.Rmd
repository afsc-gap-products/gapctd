...
The `slowdown()` module is based on the SBEDP Loop Edit (loopedit) module that flags slowdowns and reversals in scan data based on a threshold. To account for variable profiling speeds in trawl-mounted CTDs, the function also excludes the surface and bottom from flagging.
...
Note that `loop_edit()` flags slowdowns and reversals but does not remove them. As such, the plots above are identical even though slowdowns and reversals were flagged in the data:
