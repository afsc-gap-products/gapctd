...
The `wrapper_run_gapctd` function runs the `run_gapctd` function on each CTD file in the working directory. The `run_gapctd` function processes data from a single CTD data file (.cnv) using the [processing steps](/doc/batch_processing_steps.md) we have found works best for our deployments across bottom trawl survey regions. 
