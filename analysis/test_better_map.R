...
example_hauls <- RODBC::sqlQuery(channel = channel,
  ...
  akgfmaps::transform_data_frame_crs(coords = c("START_LONGITUDE", "START_LATITUDE"), 
                                     in.crs = "EPSG:4326", 
                                     out.crs = project_crs)
...
# bathy_sf <- bathy_df |>
#   sf::st_as_sf(coords = c("x", "y"), crs = project_crs) |>
#   dplyr::mutate(elevation = cut(elevation, 
#                                 c(0, 50, 100, 200, 500, 1000, 8000))) |>
...
ragg::agg_tiff(filename = here::here("paper", "plots", "map_sample_locations.tif"), 
               width = 165, 
               height = 75, 
               units = "mm", 
               res = 600)
...
map_layers <- akgfmaps::get_base_layers(select.region = "sebs", set.crs = "EPSG:4326")
