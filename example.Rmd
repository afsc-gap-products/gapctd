...
This R Markdown document demonstrates how to use the gapctd pacakge to process CTD data collected on RACE Groundfish Surveys and generated data products. This process was developed for use with SBE19plus V2 CTDs with induction pumps that are deployed on bottom-trawl survey gear in the eastern Bering Sea, Gulf of Alaska, and Aleutian Islands. Use of this package requires a connection to the AFSC RACEBASE and RACE_DATA data tables or comma separated value tables provided by AFSC staff.
...
- (a) Setting up a directory structure used for data processing.
- (b) Copying hexadecimal data (.hex) files to subdirectory 'data'. 
- (c) Copying configuration (.xmlcon) files from the user-specified directory (e.g., "G:/RACE_CTD/data/2021/ebs/v162") to subdirectory 'psa_xmlcon'.
- (d) Copying batch (.bat) processing routine files for the selected CTD model (e.g., ctd_unit = "sbe19plus") that are included in the gapctd package to the root of your working directory.
- (e) Copying program setup files (.psa) for the specified CTD model to the subdirectory 'psa_xmlcon'.
...
gapctd::calc_bottom_mean(haul_metadata_path = list.files(paste0(getwd(), "/metadata/"), full.names = TRUE),
                             pattern = "TEOS10.cnv",
...
# 7. Calculate surface temperature and salinity from each downcast

The calc_surface_mean function calculates mean temperature and salinity from downcast files in the final_cnv subdirectory. Means are calculated for a specified depth interval with the default being 2 m &lte; z &lte; 5 m (i.e., depth_interval = c(2,5)). We recommend not using the 1-m depth bin due to salinity spikes. Mean surface temperature and salinity are written to the haul metadata csv file when argument append_haul_metadata is set to TRUE.

```{r avg_surface, include=TRUE, eval=FALSE}
gapctd::calc_surface_mean(haul_metadata_path = list.files(paste0(getwd(), "/metadata/"), full.names = TRUE),
                              depth_interval = c(2,5),
                              append_haul_metadata = TRUE)
```

# 8. Move bad station data to bad_cnv directory
...
- max_diff_bt_ctd_depth: The maximum allowable difference between RACEBASE.GEAR_DEPTH while and mean CTD depth while on-bottom. This criteria handles cases where the CTD stopped collecting data during the downcast.
- min_ctd_depth: The minimum depth for a deployment to be valid. This criteria handles cases where the CTD was turned on at the surface.
...
# 9. Manually flag and remove bad data 

Filtering and correcting data using pre-specified batch processing modules fixes some sampling artifacts in the data but some problems tend to persist. For example, the thermal mass correction module despikes salinity by applying a thermal mass correction to conductivity measurements that to account for the temperature-gradient-dependent lag of the conductivity cell response, but is not always effective because different temperature gradients can lead to different response times (and therefore different corrections).
